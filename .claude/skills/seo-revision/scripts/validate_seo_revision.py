...
def locate_seo_teardown(explicit: Path | None = None) -> Path | None:
    ...
    # The seo-teardown skill is normally installed alongside this one under the
    # same skills root (…/skills/seo-revision/scripts/validate_seo_revision.py),
    # so check that sibling first, then the Claude and portable skill homes.
    ...
    for root in roots:
        if not root.is_dir():
            continue
        candidates.append(root / "seo-teardown" / "SKILL.md")
        candidates.extend(root.glob("*/SKILL.md"))
    for skill_md in candidates:
        if skill_md.is_file() and read_frontmatter_name(skill_md) == "seo-teardown":
